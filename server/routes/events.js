...
router.get('/', async (req, res) => {
  const events = await db.prepare('SELECT * FROM UserEvents WHERE userId = ? ORDER BY eventDate ASC').all(req.user.id);
  res.json(events);
});
...
router.post('/', async (req, res) => {
  ...
  const id = uuid();
  await db.prepare('INSERT INTO UserEvents (id, userId, title, details, eventDate) VALUES (?, ?, ?, ?, ?)')
...
router.delete('/:id', async (req, res) => {
  await db.prepare('DELETE FROM UserEvents WHERE id = ? AND userId = ?').run(req.params.id, req.user.id);
