...
router.post('/', async (req, res) => {
  ...
  await db.prepare('INSERT INTO GroupMembers (id, userId, groupId, role) VALUES (?, ?, ?, ?)').run(uuid(), req.user.id, id, 'creator');
...
router.post('/:id/join', async (req, res) => {
  const group = await db.prepare('SELECT * FROM StudyGroups WHERE id = ?').get(req.params.id);
  if (!group) return res.status(404).json({ error: 'Group not found' });

  const memberCount = (await db.prepare('SELECT COUNT(*) as cnt FROM GroupMembers WHERE groupId = ?').get(req.params.id)).cnt;
  if (memberCount >= group.maxMembers) return res.status(400).json({ error: 'Group is full' });

  const existing = await db.prepare('SELECT id FROM GroupMembers WHERE userId = ? AND groupId = ?').get(req.user.id, req.params.id);
  if (existing) return res.status(409).json({ error: 'Already a member' });

  await db.prepare('INSERT INTO GroupMembers (id, userId, groupId) VALUES (?, ?, ?)').run(uuid(), req.user.id, req.params.id);
  res.json({ message: 'Joined group' });
});

// UC 12 — Leave Group
router.delete('/:id/leave', async (req, res) => {
  await db.prepare('DELETE FROM GroupMembers WHERE userId = ? AND groupId = ?').run(req.user.id, req.params.id);
  res.json({ message: 'Left group' });
});

// UC 13 — Remove Member (creator only)
router.delete('/:id/members/:userId', async (req, res) => {
  const group = await db.prepare('SELECT creatorId FROM StudyGroups WHERE id = ?').get(req.params.id);
  if (!group || group.creatorId !== req.user.id) {
    return res.status(403).json({ error: 'Only the creator can remove members' });
  }
  await db.prepare('DELETE FROM GroupMembers WHERE userId = ? AND groupId = ?').run(req.params.userId, req.params.id);
...
router.post('/:id/announcements', async (req, res) => {
  ...
  const id = uuid();
  await db.prepare('INSERT INTO GroupAnnouncements (id, groupId, authorId, content) VALUES (?, ?, ?, ?)').run(id, req.params.id, req.user.id, content);
